...
- consent is per channel since eudi-dev v1.18.0: `acceptCredentialOffer(...)` and `acceptPresentationRequest(...)` accept even on a wallet started `withoutAutoAccept()`, because the API call is the caller's own consent. Use the new `submitCredentialOfferForConsent(...)` / `submitPresentationRequestForConsent(...)` to raise a consent request instead
...
- consent handling for wallets without auto-accept: `getPendingRequests()`, `approveRequest(id[, selectedClaims])`, `denyRequest(id)`, `awaitPendingRequest(timeout)`, plus `submitCredentialOfferForConsent(uri)` / `submitPresentationRequestForConsent(uri)` to submit through the interactive channel. Both return a `CompletableFuture` because the wallet holds the submission open until the request is decided
