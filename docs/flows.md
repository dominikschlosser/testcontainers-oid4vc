...
Offers can also be delivered to the wallet's own URL instead of the `openid-credential-offer://` custom scheme, for verifiers or issuers that only speak plain web URLs:
...
On a wallet started with `withVciVersion(VciVersion.V1_1)`, an authorization-code offer from an issuer publishing an `authorization_challenge_endpoint` is redeemed by presenting a credential the wallet holds, without a browser redirect. See [Configuration](configuration.md) for the startup option and the runtime switch.
